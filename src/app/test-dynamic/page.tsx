    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 16, fontFamily: 'system-ui, sans-serif', background: '#FAFBFC', color: '#023047', padding: 24 }}>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'center' }}>
        {['nike.com', 'stripe.com', 'klm.com', 'shell.com', 'kpn.com'].map(d => (
          <a key={d} href={`?company=${d}&contactname=Test`} style={{ padding: '8px 16px', borderRadius: 8, background: '#023047', color: '#fff', textDecoration: 'none', fontSize: 13, fontWeight: 600 }}>
            {d}
          </a>
        ))}
      </div>
    ...
    <div style={{ minHeight: '100vh', fontFamily: 'system-ui, sans-serif', background: '#FAFBFC' }}>
      ...
      <div style={{ background: `linear-gradient(135deg, #023047 0%, ${accentColor}22 100%)`, padding: '80px 24px', textAlign: 'center' }}>
        ...
        <div style={{ marginTop: 24, display: 'flex', gap: 12, justifyContent: 'center' }}>
          <a href="/contact" style={{ padding: '12px 28px', borderRadius: 8, background: accentColor, color: '#fff', textDecoration: 'none', fontWeight: 700, fontSize: 14 }}>
            Book a call
          </a>
          <a href="/projects" style={{ padding: '12px 28px', borderRadius: 8, background: 'rgba(255,255,255,0.15)', color: '#fff', textDecoration: 'none', fontWeight: 600, fontSize: 14, backdropFilter: 'blur(8px)' }}>
            View my work
          </a>
        </div>
      </div>

      {/* Content sections — dynamically adapted */}
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '48px 24px' }}>
        <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #E8EDF2', padding: 24, marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {[
              ...
              { title: 'Marketing Automation', desc: `Build customer journeys in Salesforce MC, Deployteq, or HubSpot — optimized for ${brand?.companyName || company}'s tech stack.` },
              ...
              <div key={i} style={{ padding: 16, borderRadius: 12, background: '#f8fafc', border: '1px solid #f4f7fa' }}>
                <h3 style={{ fontSize: 14, fontWeight: 700, color: '#023047', margin: '0 0 4px' }}>{item.title}</h3>
                <p style={{ fontSize: 13, color: '#4A6B7F', margin: 0, lineHeight: 1.5 }}>{item.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ textAlign: 'center', padding: 24 }}>
          <p style={{ fontSize: 12, color: '#8BA3B5' }}>
            This page was dynamically generated for <strong style={{ color: '#023047' }}>{brand?.companyName || company}</strong>
            {brand?.brandColor && <> using brand color <span style={{ display: 'inline-block', width: 12, height: 12, borderRadius: 3, background: brand.brandColor, verticalAlign: 'middle', marginLeft: 4 }} /></>}
          </p>
          <a href={`/test-dynamic`} style={{ fontSize: 12, color: '#EF476F' }}>Try another company</a>
        </div>
      </div>
    </div>
  );
