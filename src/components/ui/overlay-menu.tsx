...
const menuItemsData = [
  { labelNL: 'PROJECTEN', labelEN: 'PROJECTS', href: '/projects' },
  { labelNL: 'BLOG', labelEN: 'BLOG', href: '/blog' },
  { labelNL: 'GOED DOEL', labelEN: 'CHARITY', href: '/maatschappelijk' },
  { labelNL: 'ABM OUTREACH', labelEN: 'ABM OUTREACH', href: '/abm-outreach' },
  { labelNL: 'PLAYGROUND', labelEN: 'PLAYGROUND', href: '/playground' },
...
function OverlayMenuContent({ isOpen, onClose }: OverlayMenuProps) {
  ...
  const [activeImage, setActiveImage] = useState('/samba-gambia.jpg');
  ...
  const handleHover = useCallback((href: string) => {
    setHoveredHref(href);
    if (menuImages[href]) setActiveImage(menuImages[href]);
  }, []);
          ...
          <div className="relative h-full flex flex-col">
            ...
            <div className="flex items-center justify-between px-6 lg:px-12 py-5">
              <Link href="/" className="flex items-center gap-2 group" onClick={onClose}>
                <SambaLogo color="#023047" />
              </Link>
              <button onClick={onClose} className="w-11 h-11 rounded-full flex items-center justify-center transition-all duration-300 hover:rotate-90 hover:scale-110 cursor-pointer" style={{ backgroundColor: '#023047', color: '#ffffff' }} aria-label="Close menu">
            ...
            <div className="flex-1 flex items-center px-6 lg:px-12">
              <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
                {/* Menu links */}
                <motion.div className="lg:col-span-7 flex flex-col" variants={staggerContainer} initial="closed" animate="open" exit="closed">
                  {menuItems.map((item, index) => (
                    <motion.div key={item.href} variants={itemVariants} className="overflow-hidden">
                      <Link
                        ...
                        className="group flex items-center gap-3 lg:gap-4 py-0.5 cursor-pointer no-underline"
                      >
                        <span className="text-xs font-mono tracking-widest min-w-[1.5rem] lg:min-w-[2rem] transition-opacity duration-300 hidden sm:block" style={{ color: '#023047', opacity: hoveredHref === item.href ? 1 : 0.4 }}>
                          0{index + 1}
                        </span>
                        <span
                          ...
                          style={{
                            ...
                            color: hoveredHref === item.href ? '#023047' : hoveredHref ? 'rgba(2, 48, 71, 0.25)' : item.label === 'CONTACT' ? '#023047' : 'rgba(255, 255, 255, 0.9)',
                            transform: hoveredHref === item.href ? 'translateX(8px)' : 'translateX(0px)',
                          }}
                        >
                          {item.label}
                        </span>
                        <ArrowUpRight className="transition-all duration-300 hidden sm:block" style={{ width: '1.5rem', height: '1.5rem', color: '#023047', opacity: hoveredHref === item.href ? 1 : 0, transform: hoveredHref === item.href ? 'translateX(0)' : 'translateX(-8px)' }} />
                      </Link>
                    </motion.div>
                  ))}
                </motion.div>
                ...
                {/* Image - desktop only */}
                <motion.div className="hidden lg:flex col-span-5 relative items-center justify-center" initial={{ opacity: 0, x: 60 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.3, ease: [0.32, 0.72, 0, 1] as [number, number, number, number] }}>
                  <div className="relative w-full overflow-hidden" style={{ aspectRatio: '3/4', maxHeight: '70vh', borderRadius: '1.5rem', boxShadow: '0 40px 80px rgba(0,0,0,0.2)' }}>
                    <AnimatePresence mode="sync">
                      <motion.div key={activeImage} initial={{ opacity: 0, scale: 1.03 }} animate={{ opacity: 1, scale: 1, transition: { duration: 0.6, ease: [0.32, 0.72, 0, 1] as [number, number, number, number] } }} exit={{ opacity: 0, transition: { duration: 0.4 } }} className="absolute inset-0">
                        <Image src={activeImage} alt="Samba Jarju" fill className="object-cover object-top" sizes="40vw" priority />
                      </motion.div>
                    </AnimatePresence>
                  </div>
                </motion.div>
            ...
            {/* Bottom bar */}
            <motion.div className="flex items-center justify-between px-6 lg:px-12 py-5" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5, duration: 0.5 }}>
              <div className="flex items-center gap-4 lg:gap-6 text-xs lg:text-sm" style={{ color: 'rgba(2, 48, 71, 0.6)' }}>
                <span>Rotterdam, NL</span>
                <span className="hidden sm:inline">KvK: 83474889</span>
              </div>
              <div className="flex items-center gap-3">
                {[
                  ...
                  <a key={i} href={item.href} target={item.href.startsWith('http') ? '_blank' : undefined} rel={item.href.startsWith('http') ? 'noopener noreferrer' : undefined} className="w-9 h-9 rounded-full flex items-center justify-center transition-all duration-300 hover:scale-110" style={{ backgroundColor: 'rgba(2, 48, 71, 0.1)', color: '#023047' }}>
                    {item.icon}
                  </a>
                ))}
              </div>
