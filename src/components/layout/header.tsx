    ...
    >
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-2 group">
          <svg className={`h-5 sm:h-6 transition-colors duration-300 ${scrolled ? 'text-foreground' : 'text-white'}`} viewBox="36 12 940 176" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <nav className="hidden lg:flex items-center gap-8">
          {links.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className={`text-sm transition-colors duration-200 relative group ${scrolled ? 'text-foreground-muted hover:text-foreground' : 'text-white/70 hover:text-white'}`}
            >
              {link.label}
              <span className={`absolute -bottom-1 left-0 w-0 h-0.5 transition-all duration-300 group-hover:w-full ${scrolled ? 'bg-accent' : 'bg-white'}`} />
            </a>
          ))}
        </nav>
